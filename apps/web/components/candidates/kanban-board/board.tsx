...
export const KanbanBoard = ({
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    const overColumnId = (over.data.current?.columnId || over.id) as string;
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const targetColumnId = (over.data.current?.column?.id ?? over.id) as string;
    ...
    if (candidate.status?.id.toString() === targetColumn.id) {
      setColumns(previousColumns);
      return;
    }

    const update = buildCandidateVacancyUpdatePayload(candidate, parseInt(targetColumn.id));

    if (targetColumn.isRejection) {
      setPendingMove({
...
function buildColumns(
  ...
  return statuses.map((status) => ({
    ...
    candidates: candidates.filter(
      (candidate) => "status" in candidate && candidate.status?.id === status.id,
    ),
  }));
}
