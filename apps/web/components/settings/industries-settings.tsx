...
export default function IndustriesSettings() {
  ...
  const { mutate: handleAddIndustry } = useMutation({
    ...
    onSuccess: () => {
      queryClient
        .invalidateQueries({ queryKey: [INDUSTRIES_API_KEY] })
        .then(() => {
          setNewIndustry("");
  ...
  const { mutate: handleEditIndustry } = useMutation({
    mutationFn: (industry: Industry) =>
      updateIndustry(industry.id, { name: industry.name }),
    onSuccess: () => {
      queryClient
        .invalidateQueries({ queryKey: [INDUSTRIES_API_KEY] })
        .then(() => {
    ...
    <div className="space-y-4">
        ...
        {!data && isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-xs text-muted-brand mt-0.5">
              No se encontraron industrias con ese nombre.
            </p>
          </div>
        ) : (
          <ul>
      ...
      {isAddDialogOpen && (
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          <DialogContent className="sm:max-w-[420px]">
            <DialogHeader>
              <DialogTitle className="text-ink">Agregar Industria</DialogTitle>
      ...
      {!!editingIndustry && (
        <Dialog
          ...
          onOpenChange={() => setEditingIndustry(null)}
        >
          <DialogContent className="sm:max-w-[420px]">
            <DialogHeader>
              <DialogTitle className="text-ink">Editar Industria</DialogTitle>
              <DialogDescription className="text-slate-brand">
            ...
            <DialogFooter>
              ...
              <button
                onClick={() => handleEditIndustry(editingIndustry!)}
                disabled={isLoading || !editingIndustry?.name.trim()}
                className="inline-flex items-center justify-center rounded-md bg-electric px-4 py-2 text-sm font-semibold text-white transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:bg-electric-light hover:shadow-[0_8px_24px_-6px_rgba(0,102,255,0.4)] disabled:opacity-50 disabled:pointer-events-none"
              >
                {isLoading ? "Guardando..." : "Guardar"}
              </button>
