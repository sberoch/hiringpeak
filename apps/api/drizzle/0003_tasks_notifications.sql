...
--> statement-breakpoint
DO $$ BEGIN
 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tasks_completed_by_users_id_fk') THEN
 ...
 END IF;
 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tasks_assigned_to_users_id_fk') THEN
 ...
 END IF;
 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tasks_candidate_id_candidates_id_fk') THEN
  ALTER TABLE "tasks" ADD CONSTRAINT "tasks_candidate_id_candidates_id_fk" FOREIGN KEY ("candidate_id") REFERENCES "public"."candidates"("id") ON DELETE cascade ON UPDATE no action;
 END IF;
 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tasks_vacancy_id_vacancies_id_fk') THEN
  ALTER TABLE "tasks" ADD CONSTRAINT "tasks_vacancy_id_vacancies_id_fk" FOREIGN KEY ("vacancy_id") REFERENCES "public"."vacancies"("id") ON DELETE cascade ON UPDATE no action;
 END IF;
 ...
 END IF;
 IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'notifications_task_id_tasks_id_fk') THEN
