...
- **Two RBAC systems coexist; the legacy one is dormant.** `UserRole` enum (`ADMIN` / `MANAGER` / `BASIC` / `SYSTEM_ADMIN`), `RolesGuard`, and `@Roles(...)` decorator are still wired but **`@Roles` is not used anywhere in source**. The active RBAC is the DB-driven `Role` + `Permission` + `RolePermission` tables, enforced via `@Permissions(...)` + `PermissionsGuard`. Migration is in progress — see `apps/api/src/auth/ROLE_TO_PERMISSION_MAPPING.md`. New endpoints: use `@Permissions(...)` only; do not introduce new `@Roles(...)` usages.
